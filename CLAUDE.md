...
```
n8n-starter/
├── docker-compose.yml              # root compose: postgres, redis, includes n8n package
├── docker-compose.prd.yml          # production variant: no watch-server, import-only
├── .env.example                    # environment variables template
├── package.json                    # yarn workspaces root with helper scripts
├── README.md                       # project documentation
└── packages/
    └── n8n/
        ├── docker-compose.yml      # n8n, n8n-worker, n8n-init, n8n-watch-server
        ├── docker-compose.prd.yml  # production n8n services (no watch-server)
        ├── package.json            # dependencies: express, commander, slugify, yaml
...
- Manifest location: `packages/n8n/credentials/manifest.yml`
- Two sections:
...
- Location: `packages/n8n/hooks/workflowHooks.js`
...
- Dockerfile: `packages/n8n/init/Dockerfile` (node:20-alpine + n8n@2.3.5 + postgresql-client + jq)
